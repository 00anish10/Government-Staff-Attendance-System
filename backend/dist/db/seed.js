...
const seed = async () => {
    ...
    await (0, database_1.query)(`TRUNCATE TABLE leave_requests, attendance, users, staff, designations, departments RESTART IDENTITY CASCADE`);
  ...
  `);
    ...
    await (0, database_1.query)(`INSERT INTO users (username, password_hash, role, staff_id) VALUES ($1, $2, 'admin', 1)`, ['admin', hash]);
    const hrHash = await bcrypt_1.default.hash('hr123', 10);
    await (0, database_1.query)(`INSERT INTO users (username, password_hash, role, staff_id) VALUES ($1, $2, 'hr', 3)`, ['hr', hrHash]);
    const today = getNepaliNow();
    const staffResult = await (0, database_1.query)(`SELECT id, date_of_joining FROM staff`);
    ...
    for (const row of staffRows) {
        ...
        for (let d = 0; d < daysToGenerate; d++) {
            ...
            if (isSaturdayNepal(date)) {
                await (0, database_1.query)(`INSERT INTO attendance (staff_id, date, status) VALUES ($1, $2, 'holiday') ON CONFLICT (staff_id, date) DO NOTHING`, [row.id, dateStr]);
                continue;
            }
            ...
            await (0, database_1.query)(`INSERT INTO attendance (staff_id, date, check_in, check_out, status) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (staff_id, date) DO NOTHING`, [row.id, dateStr, checkIn, checkOut, status]);
        }
    }
    await (0, database_1.query)(`
